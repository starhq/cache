 ...
 */
public class ClusterRedisCacheProvider implements CacheProvider {

    private JedisCluster pool;

    private ConcurrentHashMap<String, ClusterRedisCache> redisMap = new ConcurrentHashMap<>();
    ...
    public Cache buildCache(String regionName, boolean autoCreate) throws CacheException {
        ClusterRedisCache cache = redisMap.get(regionName);
        if (Objects.isNull(cache)) {
            cache = new ClusterRedisCache(regionName, pool);
    ...
    public void start() throws CacheException {
        ...
        while (true) {
            ...
            HostAndPort hostAndPort = new HostAndPort(url, config.getInt("redis.shared.port." + i, 6379));
        ...
        if (CollectionUtil.isEmpty(hostAndPorts)) {
            throw new CacheException("create cluster redis failure,no hostand port");
        } else {
            pool = new JedisCluster(hostAndPorts, poolConfig);
        }
    }
